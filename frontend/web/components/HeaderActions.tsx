      ...
      {mobileMenuOpen && (
        <div className="fixed inset-0 z-[100] bg-background/95 backdrop-blur-md md:hidden flex flex-col pointer-events-auto">
          ...
          <nav className="flex flex-col gap-2 p-6 flex-grow overflow-y-auto">
            <Link href="/" onClick={() => setMobileMenuOpen(false)} className="px-4 py-3 rounded-lg hover:bg-white/5 text-slate-300 hover:text-white font-medium flex items-center gap-3 transition-colors">
              <FiLayout className="w-5 h-5" /> Overview
            </Link>
            <Link href="/fleet" onClick={() => setMobileMenuOpen(false)} className="px-4 py-3 rounded-lg hover:bg-white/5 text-slate-300 hover:text-white font-medium flex items-center gap-3 transition-colors">
              <FiMap className="w-5 h-5" /> Fleet Map
            </Link>
            <Link href="/docks" onClick={() => setMobileMenuOpen(false)} className="px-4 py-3 rounded-lg hover:bg-white/5 text-slate-300 hover:text-white font-medium flex items-center gap-3 transition-colors">
              <FiBatteryCharging className="w-5 h-5" /> Docks
            </Link>
            <Link href="/rides" onClick={() => setMobileMenuOpen(false)} className="px-4 py-3 rounded-lg hover:bg-white/5 text-slate-300 hover:text-white font-medium flex items-center gap-3 transition-colors">
              <MdDirectionsBike className="w-5 h-5" /> Rides
            </Link>
            <Link href="/analytics" onClick={() => setMobileMenuOpen(false)} className="px-4 py-3 rounded-lg hover:bg-white/5 text-slate-300 hover:text-white font-medium flex items-center gap-3 transition-colors">
              <FiBarChart2 className="w-5 h-5" /> Analytics
            </Link>
            <Link href="/simulator" onClick={() => setMobileMenuOpen(false)} className="px-4 py-3 rounded-lg bg-[#1ED760]/10 hover:bg-[#1ED760]/20 text-[#1ED760] border border-[#1ED760]/20 font-medium flex items-center gap-3 transition-colors mt-2">
            ...
            {(session?.user as any)?.role === 'ADMIN' && (
              <div className="mt-4 pt-4 border-t border-white/10 flex flex-col gap-2">
                ...
                <Link href="/settings" onClick={() => setMobileMenuOpen(false)} className="px-4 py-3 rounded-lg hover:bg-white/5 text-slate-300 hover:text-white font-medium flex items-center gap-3 transition-colors">
