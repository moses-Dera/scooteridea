    ...
    try {
      ...
      if (res?.error) {
        if (res.error.startsWith('2FA_REQUIRED:')) {
          ...
          setMode('2fa');
          setSuccessMsg('2FA Required. Please enter the OTP sent to your email.');
          ...
          <form onSubmit={handle2FAVerify} className="space-y-4">
            <div>
              ...
              <input
                ...
                maxLength={6}
                required
                className="w-full bg-surface border border-white/10 rounded-lg px-4 py-3 text-white focus:outline-none focus:border-primary focus:ring-1 focus:ring-primary tracking-widest text-center text-xl font-mono transition-colors"
                placeholder="000000"
              />
            </div>
            <button
            ...
            <button
              ...
              className="w-full bg-transparent border border-white/20 hover:bg-white/5 text-white font-bold py-3 rounded-lg transition-all mt-4"
            >
              Back to Login
            </button>
          </form>
