      ...
      <div className="flex overflow-x-auto pb-2 -mx-4 px-4 md:mx-0 md:px-0 gap-2 border-b border-white/10 hide-scrollbar">
        <button
          ...
          className={`flex items-center gap-2 px-4 py-3 font-medium whitespace-nowrap transition-colors border-b-2 ${
            activeTab === 'pricing' ? 'border-primary text-primary' : 'border-transparent text-slate-400 hover:text-white'
          }`}
        >
          <FiPercent />
          Pricing Engine
        ...
        <button
          ...
          className={`flex items-center gap-2 px-4 py-3 font-medium whitespace-nowrap transition-colors border-b-2 ${
            activeTab === 'geofencing' ? 'border-primary text-primary' : 'border-transparent text-slate-400 hover:text-white'
          }`}
        >
          <FiMap />
          Geofencing Zones
        ...
        <button
          ...
          className={`flex items-center gap-2 px-4 py-3 font-medium whitespace-nowrap transition-colors border-b-2 ${
            activeTab === 'users' ? 'border-primary text-primary' : 'border-transparent text-slate-400 hover:text-white'
          }`}
        >
          <FiUsers />
          User Management
        ...
        <button
          ...
          className={`flex items-center gap-2 px-4 py-3 font-medium whitespace-nowrap transition-colors border-b-2 ${
            activeTab === 'finance' ? 'border-primary text-primary' : 'border-transparent text-slate-400 hover:text-white'
          }`}
        >
          <FiDollarSign />
          Financial Ledgers
      ...
      <div className="mt-6">
        ...
        {activeTab === 'geofencing' && (
          <div className="glass-panel p-6 rounded-xl border border-white/10 flex flex-col items-center justify-center text-center py-20">
            ...
            <p className="text-slate-400 max-w-md mx-auto">
              Configure allowed riding zones, speed-limited areas, and restricted zones using the interactive map editor.
            </p>
        ...
        {activeTab === 'users' && (
          <div className="space-y-6">
            <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 mb-6">
              <p className="text-slate-400">Manage dashboard access and operator zone assignments.</p>
                ...
                <table className="w-full text-left text-sm">
                  ...
                  <tbody className="divide-y divide-slate-800/50">
                    <tr className="hover:bg-slate-800/20 transition-colors">
                      ...
                      <td className="px-6 py-4"><span className="bg-primary/10 text-primary px-2 py-1 rounded text-xs font-bold">ADMIN</span></td>
                    ...
                    <tr className="hover:bg-slate-800/20 transition-colors">
                      ...
                      <td className="px-6 py-4"><span className="bg-slate-800 text-slate-300 px-2 py-1 rounded text-xs font-bold">OPERATOR</span></td>
        ...
        {activeTab === 'finance' && (
          <div className="glass-panel p-6 rounded-xl border border-white/10 flex flex-col items-center justify-center text-center py-20">
            ...
            <p className="text-slate-400 max-w-md mx-auto">
              Manually override rider wallet balances, issue refunds, and investigate ledger anomalies.
            </p>
            <button className="mt-6 px-6 py-3 border border-slate-600 rounded-lg text-white font-medium hover:bg-slate-800 transition-colors">
              Access Ledger Logs
            </button>
