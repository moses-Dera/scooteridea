...
export default function ForgotPasswordPage() {
  ...
  return (
    <div className="min-h-screen w-full relative flex items-center justify-center p-4 sm:p-6 bg-[#0A0F1E] overflow-hidden">
      ...
      <div className="w-full max-w-lg flex flex-col rounded-3xl overflow-hidden glass-panel border border-white/5 shadow-2xl relative z-10 bg-[#1A2235]/60 backdrop-blur-xl p-8 sm:p-12">
        
        <Link href="/login" className="absolute top-8 left-8 text-slate-400 hover:text-white transition-colors flex items-center gap-2 text-sm font-medium">
        ...
        {success ? (
          <div className="flex flex-col items-center justify-center text-center animate-in fade-in zoom-in duration-500">
            ...
            <Link 
              href="/login"
              className="w-full h-14 bg-white/5 hover:bg-white/10 text-white font-bold rounded-xl flex items-center justify-center border border-white/10 transition-all"
            >
              Return to Login
            </Link>
          </div>
          ...
          <form onSubmit={handleReset} className="flex flex-col gap-6">
            ...
            <button 
              ...
              className="group relative w-full h-14 bg-[#00FFA3] text-black font-bold text-lg rounded-xl flex items-center justify-center gap-2 overflow-hidden transform hover:-translate-y-0.5 hover:shadow-[0_0_25px_rgba(0,255,163,0.4)] transition-all disabled:opacity-70 disabled:hover:translate-y-0"
            >
              {loading ? (
