          ...
          history.map((trip: any) => (
            <div key={trip.id} className="glass-panel p-5 rounded-2xl border border-white/5 hover:border-primary/30 transition-colors group cursor-pointer relative overflow-hidden">
              ...
              <div className="flex justify-between items-start mb-4">
                <div>
                  <div className="text-xs font-bold text-slate-500 mb-1">{trip.id.substring(0, 8).toUpperCase()}</div>
                  <div className="flex items-center gap-2 text-sm font-semibold text-slate-300">
                    <Calendar className="w-4 h-4 text-primary" /> {new Date(trip.startTime).toLocaleDateString()}
                  </div>
                </div>
                <div className="text-lg font-black text-white">₦ {(trip.costCents / 100).toFixed(2)}</div>
              </div>
              ...
              <div className="flex items-center gap-6 pt-4 border-t border-white/5 text-sm text-slate-400 font-medium">
                <div className="flex items-center gap-2">
                  ...
                  {trip.endTime ? Math.max(1, Math.round((new Date(trip.endTime).getTime() - new Date(trip.startTime).getTime()) / 60000)) : 0} min
                </div>
                <div className="flex items-center gap-2">
