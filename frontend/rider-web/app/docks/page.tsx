      ...
      <div className="px-6 py-6 space-y-4">
        ...
        <div className="bg-primary/10 border border-primary/20 rounded-2xl p-4 flex items-start gap-3 mb-6">
          ...
          <p className="text-sm text-primary/90 font-medium">
            You must park your scooter at a designated docking station to end your ride and avoid a penalty fee.
          </p>
        </div>
        ...
        {loading ? (
          ...
          docks.map((dock) => (
            <div key={dock.id} className="glass-panel p-5 rounded-2xl border border-white/5 hover:border-white/10 transition-colors">
              <div className="flex justify-between items-start mb-2">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <p className="text-xs text-slate-400 font-semibold">{dock.distanceKm} km away</p>
                  </div>
                </div>
              </div>
              ...
              <div className="flex items-center justify-between mt-4 pt-4 border-t border-white/5">
                <div>
                  <div className="text-xs text-slate-500 font-bold uppercase mb-1">Available Slots</div>
                  <div className={`text-xl font-black ${dock.availableSlots > 2 ? 'text-primary' : 'text-warning'}`}>
                    {dock.availableSlots} <span className="text-sm font-semibold text-slate-400">spots left</span>
                  </div>
                </div>
                
                <button 
                  onClick={() => router.push(`/?navigate=true&lat=${dock.lat}&lng=${dock.lng}`)}
                  className="px-4 py-2 bg-white/10 hover:bg-white/20 transition-colors rounded-xl font-bold text-sm flex items-center gap-2 group">
