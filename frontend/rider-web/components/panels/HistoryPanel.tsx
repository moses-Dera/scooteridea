...
import { Bike, Calendar, MapPin, Clock, ArrowLeft, X } from 'lucide-react';
  ...
  useEffect(() => {
    if (status === 'unauthenticated') {
      window.dispatchEvent(
        new CustomEvent('auth-required', { detail: { feature: 'your Ride History' } }),
      );
    }
  }, [status]);
              ...
              <div className="flex justify-between items-start mb-4">
                <div>
                  <div className="text-xs font-bold text-slate-500 mb-1">
                    {trip.id?.substring(0, 8).toUpperCase() || 'TRIP'}
                  </div>
                ...
                <div className="text-lg font-black text-white">
                  ₦ {(trip.fareCents ? trip.fareCents / 100 : trip.cost || 0).toFixed(2)}
                </div>
              </div>
