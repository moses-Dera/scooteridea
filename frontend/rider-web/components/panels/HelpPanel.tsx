'use client';
import { HelpCircle, ChevronDown, Mail, MessageSquare, ArrowLeft, X } from 'lucide-react';
...
export default function HelpPanel({ onClose }: { onClose: () => void }) {
  ...
  const handleLiveChat = () => {
    setConnecting(true);
    setTimeout(() => {
      toast.error('All our support agents are currently busy. Please leave us an email.', {
        icon: '🤖',
      });
      setConnecting(false);
    }, 1500);
  ...
  const faqs = [
    {
      q: 'How do I unlock a scooter?',
      a: 'Scan the QR code on the handlebars or enter the vehicle ID manually in the app.',
    },
    {
      q: 'Where can I ride?',
      a: 'You can ride anywhere within the operational zone shown on the map. Red zones indicate no-ride areas.',
    },
    {
      q: 'How is pricing calculated?',
      a: 'There is a standard per-minute rate. Surge pricing may apply during high demand. There is NO unlock fee.',
    },
    {
      q: 'How do I end my ride?',
      a: "Park the scooter in a designated parking zone (shown as green docks on the map) and tap 'End Ride' in the app.",
    },
  ];
    ...
    <div className="px-6 pb-6 text-white space-y-6">
      <div className="flex items-center justify-between mb-6 pt-2">
        <div className="flex items-center gap-3">
          ...
          <div className="text-2xl font-bold flex items-center gap-2">Help & Support</div>
        </div>
        <button
          onClick={onClose}
          className="w-10 h-10 rounded-full bg-white/5 hover:bg-white/10 flex flex-shrink-0 items-center justify-center transition-colors cursor-pointer"
      ...
      <div>
        <div className="text-xs font-bold text-slate-500 uppercase tracking-widest mb-3 mt-4">
        ...
        <div className="space-y-2">
          {faqs.map((faq, idx) => (
            ...
            >
              ...
              {openFaq === idx && (
                <div className="p-4 pt-0 text-sm text-slate-400 bg-white/5">{faq.a}</div>
              )}
            </div>
          ))}
        </div>
