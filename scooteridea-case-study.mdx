<Note>
  **How to use**: This case study documents the architectural decisions, trade-offs, and implementation details for the ScooterIdea platform.
</Note>
...
* **Decision 2: Ephemeral State Separation:** The `Bikes` table only stores the *last known* state. Live, second-by-second tracking exists entirely in Redis. We push historical breadcrumbs to a time-series partitioned `bike_locations` table asynchronously via a Kafka DB-Writer worker.
